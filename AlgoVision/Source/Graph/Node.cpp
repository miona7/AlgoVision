...
Node::Node()
    : m_name(""), m_id(nextId()), m_neighbours(), m_position(0.0, 0.0) {
}

Node::Node(const Node &n)
    : m_name(n.m_name), m_id(n.m_id), m_neighbours(n.m_neighbours),
      m_position(n.m_position) {
}

Node::Node(const std::string &s)
    : m_name(QString::fromStdString(s)), m_id(nextId()), m_neighbours(),
      m_position(0.0, 0.0) {
}
...
void Node::removeNeighbour(Node *node) {
    ...
    for(auto it = m_neighbours.begin(); it != m_neighbours.end();) {
        if(*it == node) {
            it = m_neighbours.erase(
                it); // vraca pokazivac na sledeci nakon izbacivanja
        } else {
            ++it;
        }
    }
